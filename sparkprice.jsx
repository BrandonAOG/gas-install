...
// The 10 suppliers this app searches. Must match the backend allowlist.
const SUPPLIERS = [
  "Grainger", "Graybar", "Rexel", "Platt", "Home Depot Pro",
  "Lowe's Pro", "Zoro", "City Electric", "Border States", "Gexpro",
  "Gordon Electric Supply", "SupplyHouse.com",
];
...
function fakeSearch(query, cat) {
  ...
  return DEMO_DB.filter((r) => {
    ...
    const matchSup = SUPPLIERS.includes(r.seller); // enforce the 10-supplier allowlist
    return matchQ && matchC && matchSup;
  });
}
  ...
  return (
    <div style={S.page}>
      ...
      <header style={S.header}>
        ...
        <div style={S.supplierStrip}>
          <span style={S.supplierLabel}>Searching {SUPPLIERS.length}:</span>
          <span style={S.supplierNames}>{SUPPLIERS.join(" · ")}</span>
        </div>
      </header>
...
const S = {
  ...
  supplierStrip: { display: "flex", gap: 8, marginTop: 12, fontSize: 11, alignItems: "baseline", flexWrap: "wrap" },
  supplierLabel: { color: "#d4a017", fontWeight: 700, letterSpacing: "0.04em", flexShrink: 0 },
  supplierNames: { color: "#8a8577", lineHeight: 1.5 },
